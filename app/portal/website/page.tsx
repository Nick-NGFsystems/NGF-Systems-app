...
// These keys are admin-only and never shown to clients
...
// Keys that should render as color picker
...
export default function WebsiteEditorPage() {
  ...
  useEffect(() => {
    const handler = (e: MessageEvent) => {
      if (e.data?.type === 'fieldClick') {
        ...
        setActiveField({ section, field, value: currentValue, label: `${humanize(section)}: ${humanize(fieldPart)}` })
  ...
  const renderSectionContent = (sectionKey: string) => {
    ...
    // Array of strings → image gallery / URL list
      ...
      return (
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            />
            <button
              onClick={() => addGalleryImage(sectionKey)}
    ...
    // Array of objects → list editor (services, testimonials, team, etc.)
    ...
    // Empty array → treat as gallery by default
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <input type="text" placeholder="Paste image URL…" value={inputVal}
              ...
              onKeyDown={e => { if (e.key === 'Enter') addGalleryImage(sectionKey) }}
              className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            <button onClick={() => addGalleryImage(sectionKey)} disabled={!inputVal.trim()} className="px-3 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-40 text-white rounded-lg text-sm font-medium transition-colors">Add</button>
          </div>
    ...
    // Object → render each string field dynamically
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex-1 relative bg-gray-100">
        <div className="absolute inset-2 bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200">
          ...
          <iframe ref={iframeRef} src={previewUrl} onLoad={enableEditMode} className="w-full border-0" style={{ height: 'calc(100% - 37px)' }} title="Website Preview" />
        </div>
