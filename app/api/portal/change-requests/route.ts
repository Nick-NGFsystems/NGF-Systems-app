...
export async function GET() {
  ...
  try {
    ...
    console.error('[portal/change-requests GET]', err)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { title, description, page_section, priority } = await request.json() as {
      title: string
      description: string
      page_section?: string
      priority?: string
    ...
    const record = await db.changeRequest.create({
      data: {
        ...
        page_section: page_section?.trim() || null,
        priority: safePriority as 'LOW' | 'MEDIUM' | 'URGENT',
        status: 'PENDING'
      },
    })
