...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    return NextResponse.json({ success: false, error: 'Failed to update recurring income' }, { status: 500 })
  }
}
...
export async function DELETE(_request: Request, context: RouteContext) {
  try {
    ...
    await db.recurringIncome.delete({
      where: { id },
    })
    ...
    return NextResponse.json({ success: false, error: 'Failed to delete recurring income' }, { status: 500 })
  }
}
