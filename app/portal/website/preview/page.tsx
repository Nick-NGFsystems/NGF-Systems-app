...
// Prevent any link/button navigation in edit mode
function noNav(e: React.MouseEvent) { e.preventDefault(); e.stopPropagation() }

function Section({ id, selected, onClick, children, className = '', style }: {
  id: string; selected: boolean; onClick: (id: string) => void
  ...
  return (
    <section
      ...
      className={`relative cursor-pointer transition-all duration-150 ${className} ${
        selected ? 'ring-2 ring-inset ring-blue-500' : 'hover:ring-2 hover:ring-inset hover:ring-blue-300'
      }`}
      style={style}
    >
      {selected && (
...
export default function PreviewPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/portal/website')
      .then(r => r.json())
      .then(data => { if (data.content) setContent(data.content as WebsiteContent) })
      .catch(() => {})
  }, [])

  useEffect(() => {
    const handler = (e: MessageEvent) => {
      if (e.data?.type === 'contentUpdate' && e.data.content) setContent(e.data.content as WebsiteContent)
      if (e.data?.type === 'scrollTo' && e.data.section) {
        document.getElementById(e.data.section)?.scrollIntoView({ behavior: 'smooth', block: 'start' })
      }
    }
  ...
  return (
    ...
    >
      {/* Nav — prevent all clicks navigating */}
      <nav className="sticky top-0 z-20 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between shadow-sm">
        ...
        <div className="hidden sm:flex gap-6 text-sm text-gray-600">
          {['About', 'Services', 'Contact'].map(label => (
            <a key={label} href={`#${label.toLowerCase()}`} onClick={noNav}
              className="hover:text-gray-900 cursor-default select-none">
              {label}
            </a>
          ))}
        </div>
        <a href="#" onClick={noNav}
          className="px-4 py-1.5 rounded-full text-white text-sm font-medium cursor-default select-none"
      ...
      <Section id="services" selected={selectedSection === 'services'} onClick={handleSectionClick}
        className="py-20 px-6" style={{ backgroundColor: '#F9FAFB' }}>
        <div className="max-w-5xl mx-auto">
          <h2 className="text-3xl font-bold mb-10 text-center" style={{ color: primary }}>Our Services</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {content.contact?.phone && (
            <div className="flex items-start gap-3">
              ...
              <div><p className="text-xs text-gray-400 font-medium uppercase tracking-wide">Phone</p><p className="text-gray-700">{content.contact.phone}</p></div>
            </div>
          )}
          {content.contact?.email && (
            <div className="flex items-start gap-3">
              ...
              <div><p className="text-xs text-gray-400 font-medium uppercase tracking-wide">Email</p><p className="text-gray-700">{content.contact.email}</p></div>
            </div>
          )}
          {content.contact?.address && (
            <div className="flex items-start gap-3">
              ...
              <div><p className="text-xs text-gray-400 font-medium uppercase tracking-wide">Address</p><p className="text-gray-700 whitespace-pre-line">{content.contact.address}</p></div>
            </div>
          )}
          {content.contact?.hours && (
            <div className="flex items-start gap-3">
              ...
              <div><p className="text-xs text-gray-400 font-medium uppercase tracking-wide">Hours</p><p className="text-gray-700">{content.contact.hours}</p></div>
            </div>
          )}
        </div>
      </Section>
      ...
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-gray-900/80 backdrop-blur text-white text-xs px-4 py-2 rounded-full shadow-lg pointer-events-none select-none">
